...
- 用 `search-layer`（Deep 模式）补充获取社区链接和非 GitHub 资源：
  ```bash
  python3 search-layer/scripts/search.py "<project_name> review OR 评测 OR 使用体验" --mode deep --num 5
  ```
  > **路径说明**: 上述命令假设你在 skills 安装根目录下执行。如果 search-layer 安装在其他位置，请替换为实际路径。
...
| Mode | 命令 | 何时用 |
|------|------|--------|
| **Fast** | `python3 search-layer/scripts/search.py "<query>" --mode fast --num 5` | 快速查找单个事实/链接 |
| **Deep** | `python3 search-layer/scripts/search.py "<query>" --mode deep --num 5` | 项目调研、竞品分析（**默认用这个**） |
| **Answer** | `python3 search-layer/scripts/search.py "<query>" --mode answer --num 5` | 需要带解释的总结型问题 |
...
调用方式：
```bash
python3 content-extract/scripts/content_extract.py --url <URL>
```
...
本 Skill 依赖以下 OpenClaw 工具和 Skills（同仓库内的 sibling skills）：

| 依赖 | 类型 | 用途 | 安装 |
|------|------|------|------|
| `web_search` | OpenClaw 内置工具 | Brave Search 检索 | 无需安装 |
| `web_fetch` | OpenClaw 内置工具 | 网页内容抓取 | 无需安装 |
| `browser` | OpenClaw 内置工具 | 动态页面渲染（备选） | 无需安装 |
| `search-layer` | Skill (本仓库) | 多源搜索去重（Brave + Exa + Tavily） | 见 `../search-layer/` |
| `content-extract` | Skill (本仓库) | 高保真内容提取（反爬站点降级方案） | 见 `../content-extract/` |
| `mineru-extract` | Skill (本仓库) | MinerU API 解析（content-extract 的下游） | 见 `../mineru-extract/` |
